...
def make_continuous_data(data_path, train_dir, test_dir):
    """
    """
    data = loadmat(data_path)['Doppler']
    label = loadmat(data_path)['Label']
    ...
    for i in tqdm(range(data_size)):
        ...
        dop_x = np.abs(raw_x) # extract doppler
        raw_y = np.zeros([seq_len, 1], dtype=np.int32)
        raw_y[:, :] = label[i][0].T - 1 # convert the indexing format
        slide_x, slide_y = sliding_window(dop_x, raw_y, overlap=0.9, win_len=50)
        data_x.append(slide_x)
        data_y.append(slide_y)
    data_x = np.vstack(data_x)
...
if __name__ == "__main__":
    # make_discrete_data('datasets/processed/discrete2.mat', 'data/Doppler_train.npy', 'data/Doppler_test.npy')
    make_continuous_data('datasets/processed/continuous.mat', 'data/Doppler_train.npy', 'data/Doppler_test.npy')